...
exports.getStudents = (name, nickName, bachelor) => {
	const searchedStudent = students.filter(student => {
		...
		let result = true;
		if (name) {
			const isFoundName = student.name.toLowerCase().includes(name.toLowerCase());
			result = result && isFoundName; // Pastikan ini benar
		}
		if (nickName) {
			const isFoundNickName = student.nickName.toLowerCase().includes(nickName.toLowerCase());
			result = result && isFoundNickName; // Pastikan ini benar
		}
		if (bachelor) {
			const isFoundBachelor = student.education.bachelor.toLowerCase().includes(bachelor.toLowerCase());
			result = result && isFoundBachelor; // Pastikan ini benar
		}
